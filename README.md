...
POST /auth/sign-in — Вход (получение токенов)

POST /api/wallets — Создать кошелек

GET /api/wallets — Список всех кошельков пользователя
...
make build — Сборка бинарного файла локально
...
make migrate-docker — Ручной накат миграций (обычно делается автоматически при старте)
